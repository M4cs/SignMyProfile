    ...
    if not user:
        ...
        if new_user.save():
            if request.cookies.get('loginandsigntarget'):
                res = make_response(redirect('/sign/' + request.cookies.get('loginandsigntarget')))
...
def sign(target):
    ...
    if request.cookies.get('auth_token'):
        ...
        return redirect('/loginandsign/' + target)

@app.route('/loginandsign/<target>')
def loginandsign(target):
    if request.cookies.get('loginandsigntarget'):
        res = make_response(redirect('/loginandsign/' + target))
        res.set_cookie('loginandsigntarget', '', 0)
        return res
